...
class MovieController {
    ...
    static async create(req, res) {
        try {
            ...
            const result = await MovieService.create({ id, title, genre, year });
    ...
    static async update(req, res) {
        try {
            ...
            if (findOne.rows.length > 0) {
                const result = await MovieService.update(id, { title, genre, year });
    ...
    static async updatePhoto(req, res) {
        try {
            ...
            if (req.file != null) {
                ...
                const file = `http://localhost:3000/upload/${imgName}`;
